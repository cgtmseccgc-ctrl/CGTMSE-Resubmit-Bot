...
def _close_page_save_video(page, evidence: Evidence | None, stem: str, log: LogFn) -> None:
    if page is None:
        return
    try:
        page.close()
    ...
    if evidence is None:
        return
    try:
        dest = evidence.save_video(page, stem)
        if dest:
            log("ok", f"Video saved: {dest.name}", "")
    except Exception as exc:
        log("warn", f"Video not saved ({stem}): {exc}", "")
...
def run_live(
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            if video_dir is not None:
                log("ok", f"Video recording on → {video_dir}", "")
        except Exception as exc:
            log("warn", f"Video context failed ({exc}) — continuing without video", "")
        ...
        try:
            ...
            try:
                if page is not None and not page.is_closed():
                    _close_page_save_video(page, evidence, "session-end", log)
            except Exception:
                pass
            try:
                context.close()
    ...
    if evidence:
        try:
            ...
            pass
        try:
            vids = sorted((evidence.dir / "video").glob("*.webm"))
            evidence.write_json("videos.json", [{"name": p.name, "bytes": p.stat().st_size} for p in vids])
        except Exception:
            pass
